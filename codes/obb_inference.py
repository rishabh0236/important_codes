...
ORTHOMOSAIC_PATH = r"D:\Rishabh Uikey\onward solar\onward 20mw\ortho\kml1\Thermal_Ortho_KML1Clipped.tif"
    ...
    with rasterio.open(ortho_path) as src:
        ...
        for col_off in range(0, src.width, tile_size - overlap):
            for row_off in range(0, src.height, tile_size - overlap):
                ...
                results = model.predict(tile_image, conf=0.4)
...
def adjust_polygon_size(polygon, width=1.1, height=2):
...
def remove_overlapping_polygons(features, threshold=0.3):
    """
...
def convert_annotations_to_geojson(geotiff_path, annotations_txt_path, output_geojson_path):
    ...
    filtered_features = remove_overlapping_polygons(features, threshold=0.3)
