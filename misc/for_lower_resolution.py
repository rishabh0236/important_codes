...
#     """
#     Extracts the filename without extension and the file extension from a given filepath.

#     Parameters:
#         filepath (str): The full path to the file.

#     Returns:
#         tuple: A tuple containing the filename without extension and the file extension.
#     """
...
#             output_path = os.path.join(output_folder, f"{raster_name}_tile_{(num_rows - row_number) * out_image.shape[1]}_{(col_number - 1) * out_image.shape[2]}.png")
...
#                 longitude, latitude = polygon_coordinate[:2]
...
#         tile_y, tile_x = int(parts[-2]), int(parts[-1])
...
# def normalisation(output_annotation_dir, root_dir, normalization_width=500, normalization_height=500):
#     """
#     Parameters:
...
#     """
#     normalized_output_dir = os.path.join(root_dir, "norm_output_annotations")
#     os.makedirs(normalized_output_dir, exist_ok=True)

#     input_files = os.listdir(output_annotation_dir)
...
#         output_file = os.path.join(normalized_output_dir, input_file)
...
#         output_label_file_path = os.path.join(labels_output_folder, file.replace(".png", ".txt"))
...
# def preprocess_data(raster_path, geojson_path, output_dir, cell_size_m=50, normalization_width=500, normalization_height=500):
#     extent = {}
#     extent_v2 = {}
...
#         payload_v2, raster_path, straight_grid_geojson, cell_size_m=cell_size_m
#     )
...
#     create_grid_from_geojson_and_raster(payload_v2, one_band_raster, straight_grid_geojson, cell_size_m=cell_size_m)
...
#         normalisation(output_annotation_dir, root_dir, normalization_width, normalization_height)
...
# raster_path = r"D:\Rishabh Uikey\onward solar\onward 40mw\ortho\KML1_thermal_clipped.tif"
# geojson_path =r"D:\Rishabh Uikey\onward solar\onward 40mw\panels\panel.geojson"
# output_dir = r"D:\Rishabh Uikey\onward solar\onward 40mw\try"

# preprocess_data(raster_path, geojson_path, output_dir, cell_size_m=50, normalization_width=500, normalization_height=500)
    ...
    with rasterio.open(raster_path) as src:
        ...
        for idx, cell in enumerate(grid.geometry):
            ...
            try:
                ...
                output_path = os.path.join(output_folder, f"{raster_name}_tile_{(num_rows - row_number)*1000}_{(col_number-1)*1000}.png")
...
def preprocess_data(raster_path, geojson_path, output_dir):
    ...
    for raster_info in converted_geojson_data_v2['raster_features']:
        ...
        split_existing_tiles_with_annotations(output_tiles, annotations_file, output_tile_dir, output_annotation_dir)
        normalisation(output_annotation_dir, root_dir, 1000, 1000)
...
raster_path = r"D:\Rishabh Uikey\onward solar\onward 40mw\ortho\KML1_thermal_clipped.tif"
geojson_path =r"D:\Rishabh Uikey\onward solar\onward 40mw\panels\panel-2.geojson"
output_dir = r"D:\Rishabh Uikey\onward solar\onward 40mw\try-1"
